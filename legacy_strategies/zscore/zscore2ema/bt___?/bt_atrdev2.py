...
def run_backtest(df, direction, threshold, sl_atr_mult, rr, spread):
    ...
    for i in range(2, len(df)):
        ...
        if not in_position:
            ...
            uptrend = prev1['EMA'] > prev1['EMA_SLOW']
            adx_entry = prev1['ADX']
...
def main():
    if len(sys.argv) != 6:
        print("Usage: python3 bt_atrdev2.py <file> <buy/sell> <threshold> <sl_atr_mult> <rr>")
        sys.exit(1)
    ...
    print(f"[INFO] Instrument: {instrument} | Spread: {spread}")
