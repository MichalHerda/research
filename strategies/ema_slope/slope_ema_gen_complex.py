...
- EMA FAST
- EMA SLOW
- EMA FAST slope normalized by ATR
- EMA SLOW slope normalized by ATR
...
def compute_ema_deviation(close, ema_fast, atr):
...
def classify_bias(ema_fast, ema_slow):
...
def classify_context(bias, trend):
    ...
    conditions = [
        (bias == 'BULL') & (trend == 'UP'),
        (bias == 'BULL') & (trend == 'DOWN'),
        (bias == 'BULL') & (trend == 'RANGE'),

        (bias == 'BEAR') & (trend == 'UP'),
        (bias == 'BEAR') & (trend == 'DOWN'),
        (bias == 'BEAR') & (trend == 'RANGE'),
    ]
...
# ============================================================
# Duration helper
# Counts consecutive identical values backward
# ============================================================

def compute_duration(series):

    durations = np.zeros(len(series), dtype=int)
...
def process_csv(
    ...
    df['ema_fast_slope_atr'] = compute_slope(
        df['ema_fast'],
        df['atr'],
        slope_period
    )

    df['ema_slow_slope_atr'] = compute_slope(
        df['ema_slow'],
        df['atr'],
        slope_period
    )
    ...
    # --------------------------------------------------------
    # Trend
    # Based on FAST EMA slope
    # --------------------------------------------------------

    df['trend'] = classify_trend(
        df['ema_fast_slope_atr'],
        trend_threshold
    )
    ...
    df['trend_duration'] = compute_duration(
        df['trend']
    )

    df['bias_duration'] = compute_duration(
        df['bias']
    )

    df['context_duration'] = compute_duration(
        df['context']
    )
...
def main():
    ...
    if os.path.exists(output_dir):
        ...
        print(
            f"[INFO] Removing existing output directory: "
            f"{output_dir}"
        )
