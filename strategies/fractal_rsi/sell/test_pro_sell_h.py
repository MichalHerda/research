...
BEGIN_NIGHT_BREAK_NY = 10  # 15:00
END_NIGHT_BREAK_NY = 21  # 19:00

# Nowa przerwa: Otwarcie Europy (8:00 - 9:00 rano czasu polskiego/bukareszteńskiego)
# 8:00 AM Europe/Bucharest to zazwyczaj 1:00 AM America/New_York (różnica 7h)
EUROPE_OPEN_BREAK_START_NY = 1
EUROPE_OPEN_BREAK_END_NY = 2
...
def is_trading_allowed(timestamp):
    ...
    # Czas lokalny brokera – tu są godziny 3-16 z .cs
    local_time = timestamp.astimezone(broker_tz)
    local_hour = local_time.hour

    # Okno handlowe: 3:00–16:00 czasu lokalnego
    if local_hour < 3 or local_hour >= 16:
        return False

    # Przerwa: 8:00–9:00 czasu lokalnego
    if local_hour == 8:
        return False

    # Opcjonalnie: nocna przerwa w NY (jeśli nadal potrzebna)
    ny_time = timestamp.astimezone(ZoneInfo("America/New_York"))
    ny_hour = ny_time.hour
    if BEGIN_NIGHT_BREAK_NY <= ny_hour < END_NIGHT_BREAK_NY:
        return False
...
def run_backtest(df, rsi_below, rr_ratio, spread, digits):
    ...
    for i in range(2, len(df)):
        ...
        if not in_position:
            ...
            entry_price = row_0['open_M5']                              # w tabelkach mamy ceny BID
            tp = row_1['last_pivot_H1'] - spread
            ...
            sl_ratio = entry_price - tp  # sluzy do obliczenia według RRR
            ...
            # --- SL CALC ---
            sl = entry_price + (sl_ratio * rr_ratio)
            sl = round(sl, digits)
            ...
            # real_tp = sl + spread  # to gdzie był SL → teraz TP
            # real_sl = tp + spread  # to gdzie był TP → teraz SL
