#!/usr/bin/env python3
"""
bar_data_continuity.py — Batch OHLCV bar data continuity checker.
...
covering every file found — meant for fast batch sanity-checking of an
entire `csv/<SYMBOL>/<SYMBOL>_<TF>.csv` tree before backtesting.

Timeframe is read from the filename suffix (..._M15.csv, ..._H1.csv, ...).
If the suffix isn't recognised, the interval is inferred from the median
spacing of the timestamps in the file itself.

Weekend closures (Fri -> Sun/Mon) are detected automatically from the
calendar and are NOT reported as missing data. Exchange holidays cannot be
inferred automatically (no embedded calendar) — optionally pass a
--holidays file (one YYYY-MM-DD per line) and those gaps will be classified
separately too. Everything else is left under "unexplained" for manual
review; that's intentionally the bucket you care about.

Per-instrument session hours (e.g. SP500 cash hours) are NOT modelled —
there are too many instruments/brokers for a safe generic default. If you
want that later, it's a clean follow-up: a SYMBOL -> (open, close) dict
feeding the same classify_gap() function.

Usage
-----
    python3 bar_data_continuity.py <input> [-o report.csv] [-t tolerance] [--holidays holidays.txt]

Example
-------
    python3 bar_data_continuity.py ~/Desktop/sample_data_2026.06.21/csv -t 1.5
    # -> ~/Desktop/sample_data_2026.06.21/csv/continuity_report.csv
"""
...
def classify_gap(t0: datetime, t1: datetime, holidays: set[date]) -> str:
    if holidays and any(t0.date() <= d <= t1.date() for d in holidays):
        return "holiday"
    if is_weekend_gap(t0, t1):
        return "weekend"
    return "unexplained"
...
def analyse_file(file_path: Path, holidays: set[date], tolerance: float) -> list[Gap]:
    """
    ...
    for t0, t1 in zip(timestamps, timestamps[1:]):
        ...
        gaps.append(Gap(
            ...
            classification=classify_gap(t0, t1, holidays),
        ))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="bar_data_continuity.py",
        description="Batch OHLCV bar continuity checker — one combined gap report for a whole folder.",
    ...
    for idx, file_path in enumerate(files, 1):
        ...
        all_gaps.extend(analyse_file(file_path, holidays, args.tolerance))
