...
# ====== CORE ======
def run_backtest(df, spread, digits, rr_ratio):
...
# ====== MAIN ======
def main(input_dir, rr_ratio):
    output_dir = os.path.join(input_dir, "tp_only_results")
    ...
    # ===== GLOBAL STATS =====
    total_tp = sum(item["TP"] for item in summary)
    total_sl = sum(item["SL"] for item in summary)
    total_trades = total_tp + total_sl

    global_winratio = (total_tp / total_trades * 100) if total_trades > 0 else 0

    summary.append({
        "instrument": "TOTAL",
        "TP": total_tp,
        "SL": total_sl,
        "WinRatio": round(global_winratio, 2)
    })
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Użycie: python3 test_cross_tp.py <DIR> <RRR>")
        sys.exit(1)

    input_dir = sys.argv[1]
    rr_ratio = float(sys.argv[2])

    main(input_dir, rr_ratio)
