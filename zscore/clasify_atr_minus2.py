...
def classify_sets(df):
    ...
    sets = {
        "all": df_down,
        "uptrend_true": df_down[df_down['uptrend'] == True],                # noqa
        "uptrend_false": df_down[df_down['uptrend'] == False]               # noqa
    }
...
def process_file(filepath, output_base_dir):
    ...
    for key, data in classified.items():
        output_filename = f"classified_minus_{key}.csv"
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 classify_atr_minus2.py <directory>")
        sys.exit(1)
